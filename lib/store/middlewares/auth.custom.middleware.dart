...
final authLoginCustomCredentialsEpic = new TypedEpic<AppState, CustomAccountLoginAction>(customCredentialsLogin);
...
Stream<dynamic> _createAccount(
  ...
  return actions.asyncMap(
      (action) => _auth.createUserWithEmailAndPassword(email: action.email, password: action.password).then((value) {
...
Stream<dynamic> _resetPassord(Stream<ResetPassword> actions, EpicStore<AppState> store) {
  return actions.asyncMap((action) => _auth.sendPasswordResetEmail(email: action.email).then((value) {
