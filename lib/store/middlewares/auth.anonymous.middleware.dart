...
final authLoginAnonymousCredentialsEpic = combineEpics<AppState>([
  ...
  new TypedEpic<AppState, EraseAnonAccountAndStartAgain>(
      _eraseAnonAccountAndStartAgain)
]);
...
Stream<dynamic> customCredentialsLogin(
  ...
  return actions.asyncMap((action) => _auth
              ...
              .then((UserCredential authResult) => authResult.user!.getIdToken().then((token) {
                    ...
                    return new CustomLoginSucceededAction("anonymous",
                        " ${authResult.user!.uid}", authResult.user!.uid, true);
                  }))
              .catchError((error) {
...
Stream<dynamic> _eraseAnonAccount(
    Stream<dynamic> actions, EpicStore<AppState> store) {
  return actions
      .where((action) => action is EraseAnonAccount)
      .asyncMap((action) {
...
Stream<dynamic> _eraseAnonAccountAndStartAgain(
    Stream<dynamic> actions, EpicStore<AppState> store) {
  return actions
      .where((action) => action is EraseAnonAccountAndStartAgain)
      .asyncMap((action) {
    return AccountApi.eraseAnonAcount().then((results) {
