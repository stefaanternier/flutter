...
final Reducer<RunState> currentRunReducer = combineReducers<RunState>([
  ...
  new TypedReducer<RunState, SyncResponsesServerToMobileComplete>(
      _addResponsesFromServer),
  new TypedReducer<RunState, SyncFileResponseComplete>(
      _addOneResponseFromServer),
  new TypedReducer<RunState, SyncActionComplete>(addOneActionFromServer),
  new TypedReducer<RunState, SyncARLearnActionsListServerToMobileComplete>(
      addActionsFromServer),
  new TypedReducer<RunState, EraseAnonAccountAndStartAgain>(resetActions),
  new TypedReducer<RunState, DeleteResponseFromLocalStore>(
      deleteResponseFromRun),
  new TypedReducer<RunState, DeleteResponseFromServer>(
      _deleteResponseFromServer),
]);
//SetCurrentRunAction
...
RunState _addMultipleChoiceResponse(
    RunState state, MultiplechoiceAction action) {
  state.outgoingResponses..add(action.mcResponse);
  return state.copyWith();
}

RunState _addResponsesFromServer(
    RunState state, SyncResponsesServerToMobileComplete action) {
  ...
  return state.copyWith(
      respFromServer: HashMap<int, Response>.from(state.responsesFromServer));
}

RunState _addOneResponseFromServer(
    RunState state, SyncFileResponseComplete action) {
  if (action.responseFromServer.responseId != null) {
    state.responsesFromServer[action.responseFromServer.responseId!] =
        action.responseFromServer;
    state.responsesFromServer =
        HashMap<int, Response>.from(state.responsesFromServer);
  }
  return state;
}

RunState deleteResponseFromRun(
    RunState state, DeleteResponseFromLocalStore action) {
...
RunState _deleteResponseFromServer(
    RunState state, DeleteResponseFromServer action) {
